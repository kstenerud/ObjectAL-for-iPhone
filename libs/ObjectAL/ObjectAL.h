 ...
 * The OpenAL 1.1 specification is available at http://connect.creativelabs.com/openal/Documentation <br>
 ...
 * Note: Changing the context will lag out the simulator if BackgroundAudio was playing and then was stopped.
 * This doesn't happen on a real device.  To avoid this issue, make sure BackgroundAudio is playing when
...
/** \mainpage ObjectAL for iPhone
 ...
 \htmlonly
 <strong>ObjectAL for iPhone</strong> is designed to be a simpler, more intuitive interface to OpenAL and AVAudioPlayer.
 ...
 All OpenAL operations can be performed using first class objects and properties, without
 needing to muddle around with arrays of data, maintain IDs, or pass around pointers to basic types.

 - BackgroundAudio provides a simpler interface to AVAudioPlayer, allowing you to play, stop, pause,
 and mute background music tracks.
 As well, it provides an easy way to configure how AVAudioPlayer will handle iPod-style music playing
 and the silent switch.
 
 - IphoneAudioSupport provides support functionality for audio in iPhone, including automatic interrupt
 handling and audio data loading routines.
 
 - SimpleIphoneAudio layers on top of the other three, providing an even simpler interface for playing background
 ...
 <a href="http://connect.creativelabs.com/openal">OpenAL API (http://connect.creativelabs.com/openal) </a>.
 ...
 - ALDevice represents a physical audio device. <br>
   Each device can have one or more contexts (ALContext) created on it, and can have multiple buffers (ALBuffer)
   associated with it.

 - ALContext controls the overall sound environment, such as distance model, doppler effect, and speed of sound. <br>
   Each context has one listener (ALListener), and can have multiple sources (ALSource) opened on it (up to a
   maximum of 32 overall on iPhone).
 
 - ALListener represents the listener of sounds originating on its context (one listener per context). It has position,
   orientation, and velocity.
 
 - ALSource is a sound emitting source that plays sound data from an ALBuffer.  It has position, direction,
   velocity, as well as other properties which determine how the sound is emitted.
 
 - ChannelSource allows you to reserve a certain number of sources for special purposes.
 
 - ALBuffer is simply a container for sound data.  Only linear PCM is supported directly, but IphoneAudioSupport load methods,
   and SimpleIphoneAudio effect preload and play methods, will automatically convert any formats that don't require hardware decoding
   (though conversion results in a longer loading time).
 
 Further information regarding the more advanced features of OpenAL (such as distance models)
 are available via the <a href="http://connect.creativelabs.com/openal/Documentation/Forms/AllItems.aspx">OpenAL Documentation at Creative Labs</a>. <br>
 In particular, read up on the various property values for sources and listeners (such as Doppler Shift) in the <strong>OpenAL Programmer's Guide</strong>,
 and distance models in section 3 of the <strong>OpenAL Specification</strong>. <br>
 Also be sure to read the
 <a href="http://developer.apple.com/iphone/library/technotes/tn2008/tn2199.html#//apple_ref/doc/uid/DTS40007999">OpenAL FAQ from Apple</a>.
 ...
 <a href="http://developer.apple.com/iphone/library/documentation/iphone/conceptual/iphoneosprogrammingguide/AudioandVideoTechnologies/AudioandVideoTechnologies.html">specified by Apple here</a>.
 ...
 <ol>
	<li>Copy libs/ObjectAL from this project into your project.  You can simply drag it into the "Groups & Files"
 section in xcode if you like (be sure to select "Copy items into destination group's folder"). <br/>
 Alternatively, you can build ObjectAL as a static library (as it's configured to do in this project).<br/><br/></li>
 ...
 <br/>
 <strong>Note:</strong> The demos in this project use <a href="http://www.cocos2d-iphone.org">Cocos2d</a>,
 a very nice 2d game engine.  However, ObjectAL doesn't require it.  You can just as easily use ObjectAL in your
 Cocoa app or anything you wish.
 <br/> <br/>
 <strong>Note #2:</strong> You do NOT have to provide a link to the Apache license from within your application.
 Simply including a copy of the license in your project is sufficient.
 \endhtmlonly
 ...
 -# Build the "ObjectAL Doc" target in this project.
 ...
 By far, the easiest component to use is SimpleIphoneAudio.  You sacrifice some power for ease-of-use,
 but for many projects it is more than sufficient.
...
@end
 ...
 Try them out and go through the code to see how it's done.  I've done my best to keep the code readable.
 Really!
 ...
 The simulator is buggy.  It can run faster or slower than a real device.  It fails system calls that
 a real device doesn't.  It shows graphics glitches that a real device doesn't.  Sounds stop working.
 ...
 When using BackgroundAudio (or SimpleIphoneAudio) to play BG music, you will see the following
 error in the debug output, which can safely be ignored:
 - MyProject[39621:207] Error: BackgroundAudio: Unknown session error (error code ffffffce)
 .
 
 Other weird errors can crop up from time to time as well.  If you're unsure, run it on a real device
 ...
 you've installed on your mac can cause the simulator to stop playing bg music, or effects, or both!
 ...
 then close the current context, the simulator will freeze (a real device won't).
 ...
 (which will in 99.9% of cases last for the entire duration of your program).  You can have a look
 inside SimpleIphoneAudio "dealloc" method to see how to avoid this problem in case you really do
 need to deallocate a device.
